...
(Based on the One Hot Gym Prediction Tutorial.)
"""
import os
import pprint
...
INPUT_FILE='san-diego.csv'
DESCRIPTION = (
  "This script runs a swarm on the san diego weather and power consumption\n"
  "It's based on the hot gym tutorial and steals liberally from it\n"
...
def writeModelParamsToFile(modelParams, name):
  "cleans up file name, adds directory if required, returns path"
  ...
  with open(outPath, "wb") as outFile:
   ...
   #    outFile.write("MODEL_PARAMS = \\\n%s" % modelParamsString)
    outFile.write("MODEL_PARAMS = %s\n%s" % (os.path.sep, modelParamsString))
  return outPath
